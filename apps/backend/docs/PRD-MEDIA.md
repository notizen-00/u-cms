...
interface Media {
  ...
  height: number | null;
  wpId: number | null;       // wp:post_id asal (post_type=attachment), null kalau bukan hasil import WordPress
  wpGuid: string | null;     // <guid> asal di WordPress, null kalau bukan hasil import
  createdAt: string;         // ISO timestamp
  updatedAt: string;
}
```

`wpId`/`wpGuid` disiapkan untuk fitur **import dari WordPress** (lihat §10) — field ini bukan sesuatu yang user isi manual lewat form upload/edit, murni ditulis oleh importer. UI Phase 1 boleh tampilkan sebagai badge read-only ("Diimpor dari WordPress") kalau `wpId` tidak null, tapi tidak wajib menyediakan input untuk field ini di form manapun.
...
# 10. WordPress Import Readiness (skema saja, importer belum dibangun)

Backend sudah menambahkan kolom provenance WordPress di tabel `media`, `news`, `pages`, `categories`, dan `tags` — **bukan** cuma di `media`, karena satu file WXR (`.xml` export WordPress) berisi post, page, attachment, category, dan tag sekaligus dan saling mereferensikan lewat ID:

| Tabel | Kolom baru | Sumber di WXR |
|---|---|---|
| `media` | `wpId` (int, nullable), `wpGuid` (text, nullable) | `<wp:post_id>` + `<guid>` pada `<item>` dengan `<wp:post_type>attachment</wp:post_type>` |
| `news` | `wpId`, `wpGuid` | sama, `post_type` = `post` |
| `pages` | `wpId`, `wpGuid` | sama, `post_type` = `page` |
| `categories` | `wpId` | `<wp:term_id>` pada elemen `<wp:category>` |
| `tags` | `wpId` | `<wp:term_id>` pada elemen `<wp:tag>` |

Semua `wpId` unique per `(siteId, wpId)` (bukan global) — dua site berbeda bisa masing-masing import dari WordPress terpisah tanpa bentrok, dan re-import file yang sama bisa dibuat idempotent (upsert by `wpId` alih-alih selalu insert baru).

**Belum termasuk di sini**: modul importer (parse XML WXR, resolve relasi `wp:post_parent`/`_thumbnail_id`/kategori-tag, download file attachment dari URL asal lalu upload ulang ke MinIO) belum diimplementasikan — ini kolom persiapan skema saja. Untuk Nuxt admin, artinya **tidak ada endpoint import yang bisa dipanggil di Phase 1 ini**; §6–§9 di atas (upload manual, grid, picker) tetap satu-satunya cara mengisi Media Library sampai importer-nya ada. Field `wpId`/`wpGuid` di response API (§7) akan selalu `null` untuk item yang diupload manual lewat UI.
