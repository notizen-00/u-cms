 ...
 * `Layout.svelte`'s `.reveal`/`.reveal.is-visible` rules do the actual
 * fade/slide transition. Falls back to marking everything visible
 * immediately if `IntersectionObserver` isn't available, so content is
 * never stuck hidden.
 */
